...
void HighWarlock::CreateAbyssFieldAndAddedToScene()
{
	...
	pAbyssField->GetTransform()->SetGlobalOffset(Vec2(0.f, 60.f));
	pAbyssField->AddComponent(make_shared<Animator>());
	...
	pAbyssField->GetAnimator()->AddAnimation(L"HighWarlock_AbyssField", pAnimation);
	pAbyssField->GetAnimator()->Play(L"HighWarlock_AbyssField", false);

	pAbyssField->Awake();
	GET_SINGLE(EventManager)->AddEvent(make_unique<ObjectAddedToSceneEvent>(pAbyssField, eSceneType));
}
